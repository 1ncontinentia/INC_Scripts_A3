...
if (_percentage > 30) then {
    ...
    if ((_percentage > (random 100)) && {_undercoverUnit getVariable ["isUndercover", false]} && {!(_undercoverUnit getVariable ["INC_undercoverCompromised", false])}) then {
        ...
        if (_undercoverGroup < (2 + (random 5))) then {
            ...
            private _civComment = selectRandom ["This is for Cyril.","I'll get my fighting hat on.","I'm bored, why not.","I'll join you, but only because they ran over my rabbit.","I should really know better.","I'm going to regret this.","Fuck it, let's go.","My wife is going to kill me for this.","Well it's better than gardening."];
            [[_civ, _civComment] remoteExec ["globalChat",0]];
            (group _civ) setGroupOwner (owner _undercoverUnit);
            [_civ,_undercoverUnit] remoteExecCall ["INCON_fnc_recruitSuccess",_undercoverUnit];
